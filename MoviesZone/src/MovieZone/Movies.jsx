        ...
        <>
            <div className='my-3 ' style={{ width: '1000px',margin:'auto'}}>
                <div className='mx-auto text-center'>
                    <button onClick={()=>setMovieList(movies)} type="button" className="btn btn-outline-secondary mx-3">All</button>
                ...
                {movielist.map((data) => (
                    <div key={data.id} style={{ maxWidth: '280px', textAlign: 'center' }}>
                        <div style={{ padding: '10px' }} className='hover-effect'>
                            <img src={data.poster_path} alt="" style={{ width: '200px', height: '280px', borderRadius: '20px', border: '1px solid orange' }} />
                        </div>
                        <h5>{data.title}</h5>
